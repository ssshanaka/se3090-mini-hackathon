...
const districts = ['All Districts', 'Colombo', 'Gampaha', 'Kalutara'];
...
function IncidentQueue() {
  ...
  const filteredReports = useMemo(() => {
    ...
    return reports.filter((report) => {
      const matchesDistrict = district === 'All Districts' || report.district === district;
      const searchableText = [report.hazardType, report.mohDivision, report.district, report.addressNotes]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
      return matchesDistrict && (!query || searchableText.includes(query));
    });
  }, [district, reports, search]);

  const filtersAreActive = district !== 'All Districts' || Boolean(search.trim());
  const clearFilters = () => {
    ...
    <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_13rem]">
          ...
          <label className="block text-sm font-semibold text-slate-700" htmlFor="district-filter">
            District
            <select
              id="district-filter"
              value={district}
      ...
      {!loading && !hasError && filteredReports.length === 0 && (
        <div className="mt-8 rounded-xl border border-slate-200 bg-white p-6 text-center shadow-sm">
          ...
          {filtersAreActive && <button onClick={clearFilters} className="mt-3 text-sm font-bold text-brand-700 underline underline-offset-2 focus:outline-none focus:ring-2 focus:ring-brand-600">View all districts</button>}
        </div>
      )}
