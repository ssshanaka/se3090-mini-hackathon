...
export default function LoginPage() {
  ...
  const submit = async (event) => { event.preventDefault(); const nextErrors = validateLogin(values); setErrors(nextErrors); setFormError(''); if (Object.keys(nextErrors).length) return; setProcessing(true); const success = await login(values.email, values.password); if (!success) { setFormError('The email address or password is incorrect.'); setProcessing(false); return; } navigate('/dashboard'); };
